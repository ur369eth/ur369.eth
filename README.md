...
On the edge of the coin, it is a self-organizing distributive-system as in the process of protecting the native tokens plus issuing-and-sending urTokens (ultra resistant Tokens) to the user's wallet >> the system shows (upfront) to the user a benefaction-fee for the service provided (i.e., a fix 0.369% of the amount to be deposited/protected) from which, if the user consents and pays the service-fee to protect their native tokens, then when the system receives the benefaction-fee, it programmatically distributes the 0.369% into 4 addresses as follows (A) 30% to fund public goods (so far on payroll Giveth, Gitcoin and the Giving Block -more can be added later-), (B) 30% to award randomly selected users every 369 hours, (C) 30% to self-sustain ur369.eth (the social good protocol), and (D) 10% to support the Community and Developers Benefit Fund.
